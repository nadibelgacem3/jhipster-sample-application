...
export class TVAItemComponentsPage {
  createButton = element(by.id('jh-create-entity'));
  deleteButtons = element.all(by.css('jhi-tva-item div table .btn-danger'));
  title = element.all(by.css('jhi-tva-item div h2#page-heading span')).first();
...
export class TVAItemUpdatePage {
  pageTitle = element(by.id('jhi-tva-item-heading'));
  ...
  tvaIDInput = element(by.id('field_tvaID'));
  ...
  async setTvaIDInput(tvaID: string): Promise<void> {
    await this.tvaIDInput.sendKeys(tvaID);
  }

  async getTvaIDInput(): Promise<string> {
    return await this.tvaIDInput.getAttribute('value');
  }
...
export class TVAItemDeleteDialog {
  private dialogTitle = element(by.id('jhi-delete-tVAItem-heading'));
  private confirmButton = element(by.id('jhi-confirm-delete-tVAItem'));
