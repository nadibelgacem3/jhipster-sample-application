...
export class EmployeeService {
  public resourceUrl = SERVER_API_URL + 'services/mooincompanies/api/employees';
  ...
  protected convertDateFromClient(employee: IEmployee): IEmployee {
    const copy: IEmployee = Object.assign({}, employee, {
      dateOfborth: employee.dateOfborth && employee.dateOfborth.isValid() ? employee.dateOfborth.format(DATE_FORMAT) : undefined,
      dateOfRecruitment:
        employee.dateOfRecruitment && employee.dateOfRecruitment.isValid() ? employee.dateOfRecruitment.format(DATE_FORMAT) : undefined,
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.dateOfborth = res.body.dateOfborth ? moment(res.body.dateOfborth) : undefined;
      res.body.dateOfRecruitment = res.body.dateOfRecruitment ? moment(res.body.dateOfRecruitment) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((employee: IEmployee) => {
        employee.dateOfborth = employee.dateOfborth ? moment(employee.dateOfborth) : undefined;
        employee.dateOfRecruitment = employee.dateOfRecruitment ? moment(employee.dateOfRecruitment) : undefined;
      });
    }
    return res;
  }
